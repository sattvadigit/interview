...
class TicTacGame {
    ...
    private $endTimee = null;
    private $totalTimee = null;
    ...
    public function makeMove($x, $y) {
                ...
                empty($this->field[$x][$y]))
        {
                ...
                // Выполняет ход AI
                if ( $current == 1 ) $this->generateAiMove();
        }
    }
     ...
     * Генерируется ход AI.
     *
     * Здесь можно детально проработать алгоритм и сложность AI
     */
    public function generateAiMove() {
        $x = 0;
        $y = 0;

        while ( !empty($this->field[$x][$y]) ) {
    ...
    public function getPlayTime()      { return $this->totalTime; }
}